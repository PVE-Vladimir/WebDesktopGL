...
#         echo "Error: Install ImageMagick (sudo apt install imagemagick or sudo pacman -S imagemagick)"
#         exit 1
#     fi
...
#         echo "Error: Install xdotool (sudo apt install xdotool or sudo pacman -S xdotool)"
#         exit 1
#     fi
}
...
if [ -f "$SCRIPT_DIR/xdotool_xseticon" ]; then
        ...
        #sudo rm -f $SCRIPT_DIR/xdotool_xseticon.c  # Cleaning
                ...
                tee "$SCRIPT_DIR/xdotool_xseticon.c" > /dev/null << EOF
                #include <stdio.h>
                #include <stdlib.h>
                #include <string.h>
                #include <X11/Xlib.h>
...
#while ! sudo xdotool search --name "$title_InfoDoc" > /dev/null 2>&1; do
...
while true; do
    ...
    window_id=$("$SCRIPT_DIR/xdotool_xseticon" search --pid "$pid" );

    for id in $window_id; do
        window_icon=$("$SCRIPT_DIR/xdotool_xseticon" _NET_WM_ICON "$id");
        if [[ "$window_icon_old" != "$window_icon" ]]; then
                #xprop -id "$id" _NET_WM_ICON
                #echo "ID windows Chromium: $window_id"
                "$SCRIPT_DIR/xdotool_xseticon" xseticon $id $input_png
                window_icon=$("$SCRIPT_DIR/xdotool_xseticon" _NET_WM_ICON "$id");
        fi
    done

    window_icon_old="$window_icon";
